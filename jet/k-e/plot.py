...
def _load_data(time):
    ...
    for k, _ in files.items():
        filepath = os.path.join(DATA_PATH, data_time,
                '{}_center-plane.raw'.format(k))
        if not os.path.exists(filepath):
            continue
        files[k] = np.loadtxt(filepath)
    print('Done.')
    sys.stdout.flush()
...
def _plot_nut(t, nut):
    ...
    plt.figure()
    plt.viridis()
    plt.tricontourf(x, y, nut/15.11e-6, 256)
    plt.gca().set_aspect('equal')
    plt.colorbar(orientation='horizontal')
    plt.xlabel('x, m')
    plt.ylabel('y, m')
...
def _plot():
    ...
    t, U, p, nut = _load_data(args['time'].pop())
